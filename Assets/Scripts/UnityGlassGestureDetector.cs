...
public class UnityGlassGestureDetector : MonoBehaviour
{
    ...
    protected virtual void Update()
    {
        ...
        if (isFocusOfTaps == true)
        {
            ...
            SWIPE_DISTANCE_THRESHOLD_PX = 0.1f;
            SWIPE_VELOCITY_THRESHOLD_PX = 0.1f;
            ...
            {
                float timeDifference = (DateTime.Now - startTime).Milliseconds;
                Debug.Log("Swiped with mouse");
                //calculate the velocity of the touch to help determien whether it was a swipe or a tap
                float velocityX = (mouseDeltaPosition.x - Input.mousePosition.x) * timeDifference;
                float velocityY = (mouseDeltaPosition.y - Input.mousePosition.y) * timeDifference;
